...
def write_assignment_fits_tile(asgn, fulltarget, overwrite, params):
    ...
    if len(tgids_avail) > 0:
        ...
        fdata = np.empty(nloc, dtype=assign_dtype)
        fdata["LOCATION"] = locs
        ...
        assigned_tgx = np.empty(nloc, dtype=np.float64)
        assigned_tgy = np.empty(nloc, dtype=np.float64)
        assigned_tgra = np.empty(nloc, dtype=np.float64)
        assigned_tgdec = np.empty(nloc, dtype=np.float64)
